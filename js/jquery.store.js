...
(function (jQuery) {
	...
	jQuery.Shop.prototype = {
		init: function () {
			...
			this.cartPrefix = "Medicine-"; // Prefix string to be prepended to the cart's name in the session storage
			...
			this.jQuerybkashForm = this.jQueryelement.find("#bkash-form"); // bkash form
			...
			this.bkashCurrency = "TAKA"; // bkash's currency code
			this.bkashBusinessEmail = "monjurulislam327@email.com"; // Your Business bkash's  account email address
			this.bkashURL = "https://www.bkash.com/"; // The URL of the bkash's  form
		...
		// Appends the required hidden values to the bkash's  form before submitting

		populatebkashForm: function () {
			var self = this;
			if (self.jQuerybkashForm.length) {
				var jQueryform = self.jQuerybkashForm;
				...
				jQueryform.attr("action", self.bkashURL);
				jQueryform.find("input[name='business']").val(self.bkashBusinessEmail);
				jQueryform.find("input[name='currency_code']").val(self.bkashCurrency);
